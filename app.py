...
class EditForm(FlaskForm):
    ...
    remember = BooleanField('remember me')
    ccnumber = StringField('ccnumber', validators=[InputRequired(), Length(min=14, max=18)])
    username = StringField('username', validators=[InputRequired(), Length(min=4, max=15)])
    password = PasswordField('password', validators=[InputRequired(), Length(min=8, max=80)])
...
def edit_profile():
    ...
    if form.validate_on_submit():
      ...
      cur.execute("""
           ...
           WHERE username=%s
           """, (form.address.data, form.name.data, form.ccnumber.data,hashedPassword, form.username.data))
      mysql.connection.commit()
      cur.close()
      return render_template('edit_profile_2.html')
